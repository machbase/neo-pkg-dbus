...
LS Job은 `ls-plc-device/get-device-data` 고정 Method Call을 하나 이상 가질 수 있다. Backend의 `DataCount`와 한 Job cycle의 전체 Tag 행은 DBus `uint16` 범위인 1~65,535까지 허용한다. LS FE만 X/B 4,096, W 2,048, D 1,024, L 512의 설정 상한을 적용한다. 따라서 generic의 `maxGeneratedTagsPerCall` 1,000, `maxBufferedRowsPerCycle` 10,000, 512 KiB Job JSON 제한은 LS Backend에 적용하지 않으며, LS Job JSON은 16 MiB까지, Job create/update/validate request body는 envelope 여유를 포함해 17 MiB까지 허용한다.
...
generic은 Job 하나당 `_dbu_<jobName>` service 하나를 사용한다. LS는 `_dbu_collector` daemon service 하나만 사용하며 Job Start/Stop은 Unix socket control command로 daemon에 전달한다. LS package `install`은 Job 유무와 관계없이 daemon service를 등록하고 시작하지 않는다. package `start`는 daemon 하나만 시작하며 활성 Job 목록을 복구한다. package `stop`은 daemon 하나만 정지하고 활성 목록을 보존한다. package `uninstall`은 daemon을 먼저 정지하고 Job 설정을 정리한 뒤 service를 해제한다. 마지막 logical Job 삭제도 service를 해제하지 않는다. LS daemon의 Job별 reader는 dedicated persistent DBus connection 하나를 가지고, 여러 Job은 같은 DBus Destination이어도 동시에 읽을 수 있다. 모든 reader는 shared bounded queue에 batch를 넣고 writer goroutine 하나만 native appender를 열고 append/flush/close한다. logical Job Start는 설정 검증과 desired-active checkpoint, runtime `starting` 저장까지만 동기 수행하고 Start control에 즉시 성공을 반환한다. 백그라운드 준비 goroutine은 writer에 Appender 연결·table column 확인·batch policy 설정을 요청한 뒤 Table METADATA의 기존 TAG 이름을 한 번 읽고 Job 설정 중 누락된 TAG만 명시적으로 등록한다. TAG 등록은 `(server, table)` 단위로 직렬화하지만 별도 SQL connection에서 실행하여 이미 실행 중인 Job의 단일 Appender writer를 막지 않는다. 준비 완료 뒤에만 runtime을 `running`으로 바꾸고 reader를 시작한다. 준비 실패는 desired-active를 제거하고 `failed`와 실제 사유를 남긴다. `starting` 중 Stop은 준비 context를 취소하고 `stopped`로 전환한다. 첫 정시 read는 writer의 append와 durable flush까지 직렬로 완료한 후 반복 schedule을 열어 first-flush 비용도 queue에 누적되지 않게 한다. 이 구간에는 dummy data나 고정 sleep을 사용하지 않는다. 한 Job Stop은 해당 reader cancel 뒤 in-flight read와 이미 queue에 넣은 batch의 append 완료까지 기다린다.
...
LS의 canonical Job 설정은 `conf.d/jobs/<name>.json`이고, 저장 완료 표시는 작은 `conf.d/job-index/<name>.json`이다. `conf.d/go-collector.json`은 logging/writer/performance 전역 정책만 가지며 Job이나 DB 연결을 복제하지 않는다. DB password와 shared table/column mapping은 `conf.d/go-collector-secrets.json`(0600)에 분리한다. Database Profile의 table/column mapping이 LS의 현재 공용 값이며, 경량 Job status와 Data Viewer도 index나 Job 안의 과거 복사본보다 이 공용 값을 우선한다. logical Job의 started/stopped 의도 상태는 `conf.d/go-collector-active-jobs.json`에 보존하고 Go daemon만 갱신한다. Go는 Start/Reload에서 index와 revision이 일치하는 canonical Job을 직접 읽고 runtime 설정을 검증한다. daemon runtime checkpoint는 `data/go-collector-runtime.json`, control socket은 `data/neo-dbus-collector.sock`이다. `refresh-log <jobName>` control도 canonical Job의 `log`만 다시 읽어 daemon의 mutex 보호 policy map에 교체한다. reader/writer·DBus connection·scheduler·overrun 값은 멈추거나 초기화하지 않는다. password와 원본 DBus body는 API, checkpoint, log에 넣지 않는다.

내부 `ls.performance`는 기본 활성화하며 Job 1,000 cycle마다 Job 이름과 DBus latency
`min/p50/avg/p99/max`, parse 평균, interval 초과와 error 수를 microsecond 단위로
한 줄 기록한다. shared writer는 Job 이름 없이 `scope=shared`로 표시하고 30초마다 channel 대기 batch 최대값,
append+flush busy ratio, enqueue부터 성공 Flush까지 p99를 한 줄 기록한다. durable
latency 표본은 최근 4,096개만 유지한다. 이 설정과 통계는 public Settings/API에
...
generic은 `lastRun`을 service details에 덮어쓴다. LS는 `data/go-collector-runtime.json`에 Job별 state, 시작/실패 설명 `stateDetail`, `lastReadAt`, `lastStoredAt`, `lastError`, `overrunCount`, `queueSkipped`, `lastOverrunAt`, 누적 rows를 보관한다. 원본 body, 추출 값, Tag 값은 저장하지 않는다. LS hot path는 memory만 갱신하고 전용 checkpoint goroutine이 변경을 최대 250ms 단위로 합쳐 원자 저장한다. Start 수락의 `starting`, 준비 완료의 `running`, Stop/Clear는 즉시 checkpoint한다. `starting` 동안 `lastReadAt`과 `lastStoredAt`은 이전 수집 값을 갱신하지 않는다.

Job monitoring과 Data Viewer의 DB 요약은 `GET /job/status?name=`을 사용한다. LS 응답은 index 기반 Job 요약과 service/runtime 기반 `lastRun`을 한 번에 반환하며 full Tag 배열을 읽지 않는다. Edit만 `GET /job`으로 canonical 설정 전체를 읽는다. generic의 마지막 Method별 결과 계약은 기존 `lastRun`을 유지한다.
...
실행 중에는 일반 Job update와 delete를 하지 않는다. `RUNNING`, `STARTING`, `STOPPING`이면 일반 Job PUT/DELETE는 `JOB_RUNNING`으로 거부한다. LS만 `PUT /job/log?name=`의 `{revision, level}`을 허용하며, 이 요청은 canonical 설정과 index 저장 뒤 `refresh-log` control로 level만 hot-apply한다. Backend는 data-plane 변경을 stop-save-start로 우회하지 않는다. 사용자는 Log Level 외 설정을 바꾸려면 먼저 stop하여 `STOPPED` 또는 `FAILED`가 된 것을 확인한다.
...
이전 배포판의 directory canonical lock도 호환해서 읽는다. fresh/live directory owner는 그대로 보호하고, lease 만료와 dead PID가 확인되면 guard 안에서 기존 entry를 정리하고 같은 경로에 file lock을 생성한다. 정상 file lock 획득·release와 legacy 회수에는 directory rename이 없다. 이 구조는 PLC Docker `/work` directory rename `ENOENT`와 무관하게 동작한다.

Neo ZIP installer의 Docker directory rename 수정은 완료됐지만 아직 정식 release 전이다. 영향 Neo 버전에서는 ZIP을 수동 해제한 뒤 lifecycle을 실행하고, 수정본이 포함된 release부터 정상 ZIP 설치를 사용한다. 상세한 실장비 특성과 진단 경로는 `docs/DEVELOPMENT.md`의 **PLC Docker `/work` operation-lock compatibility**를 따른다.

| 먼저 실행된 요청 | 뒤 요청 | 결과 |
|---|---|---|
| PUT | PUT | 실제로 lock이 겹치면 뒤 요청 `JOB_CONFLICT`; 앞 요청 종료 뒤 stale client revision은 server 최신 revision 기준으로 저장 |
...
Job PUT의 client `revision`은 호환을 위한 정보성 필드이며 생략·stale을 허용한다. Backend는 Job lock 안에서 읽은 server revision으로 repository CAS를 수행하고 실제 변경에만 다음 revision을 발급한다. 검증 후 config가 같은 재요청은 쓰기 없이 현재 상세를 반환한다. Repository CAS와 Job/package/interface mutation lock의 실제 경쟁 보호는 유지한다.
...
| `GET /job/status` | monitoring용 Job 요약과 latest runtime |
...
LS `GET /job/list`는 `job-index` 디렉터리와 daemon overview를 각각 한 번 읽고 각 항목의 `name`, `interfaceIds`, `methodCallCount`, `tagCount`, `configState`, `executionState`, `controllerState`, `controllerDetail`, `statusKnown`, `lastStoredAt`를 반환한다. Controller 상태를 알 수 없으면 `configState`와 `executionState`는 `null`, `statusKnown`은 `false`다. `GET /job/status`는 같은 index config 요약을 `job`에, 마지막 runtime projection을 `lastRun`에 반환한다. GET/POST/PUT Job 상세 성공 data는 `name`, `config`, `revision`, `configState`, `executionState`, `statusKnown`, `controllerState`, `controllerDetail`을 항상 반환하며, `config`에는 name이 없다. Job POST와 정지된 Job PUT은 Job mutation lock 안에서 database Table 존재 여부를 항상 확인한다. 없는 Table은 최종 Output Mapping에 문자열 계열 저장이 있으면 `VALUE`와 `STR_VALUE`, 없으면 `VALUE`만 포함한 TAG Table로 만들고 config도 같은 값으로 정규화한다. 생성 실패 시 config·service는 바꾸지 않는다. `/job/validate`와 runtime Start는 Table을 만들지 않는다. LS Job POST는 canonical full JSON을 원자 저장하고 index를 마지막에 저장한 뒤 shared daemon 설치 상태를 확인한다. 별도 install API는 없다.
...
- `GET /job?name=<jobName>`과 Job create/update 응답은 현재 정수 `revision`을 포함한다. `PUT /job?name=<jobName>` 본문은 name 없는 부분 config patch이고 호환용 `revision`은 선택이다. 본문에 name이 있으면 HTTP 409 `JOB_NAME_IMMUTABLE`로 거부하지만 revision 생략·stale은 허용한다. `PUT /job/log?name=<jobName>`은 LS 전용이며 `level`과 선택 `revision`만 허용한다. 두 요청 모두 **6.1 Job mutation operation lock과 lease**의 `operation lock`을 상태 조회부터 설정 파일 변경 완료까지 유지한다. 저장은 lock 안에서 읽은 server revision으로 repository CAS를 수행하고, config가 실제로 바뀐 경우에만 revision을 1 증가시킨다. 일반 Job PUT은 `defaults → existing config → request patch` 순서로 깊이 병합하고 배열은 patch가 보낸 배열 전체로 교체한다. log PUT은 현재 config의 `log.level`만 교체해 전체 config를 검증·저장한다.
- 저장 파일 `<jobName>.json`의 top-level `name`은 반드시 파일명과 같아야 한다. LS는 canonical full과 `job-index/<jobName>.json` 쌍으로 등록을 표현한다. 이전 형식 자동 migration은 하지 않으며 처음부터 index가 없는 full-only Job은 list에서 숨기고 직접 조회하면 `JOB_INVALID_CONFIG`으로 다시 생성하라고 안내한다. 같은 이름 Create는 lock 안에서 orphan을 삭제하고 새로 만든다. Create/Update는 full→index, Delete는 full 삭제→index 삭제 순서다. Update와 Log Level Update는 새 index 저장 전에 기존 index를 삭제하지 않는다. 따라서 index write가 실패해도 이전 등록 marker가 남고 canonical 상세는 계속 조회할 수 있다. 동일 config 재시도는 revision을 올리지 않으면서 canonical과 다른 stale/invalid index를 다시 쓴다. 이 중단·복구 규칙은 `DBUS_SDD.md` CCR-086을 따른다.
...
- generic `POST /job/start`은 Interface/Method/DB 설정을 JSH에서 재검증한다. LS Start/Stop JSH는 index로 등록 여부만 확인하고 shared daemon 상태 확인 뒤 Job name control만 Unix socket으로 전달한다. Go가 Start/Reload에서 canonical full JSON, index revision, DB secret/profile mapping과 Call runtime 구조를 검증한다. 같은 Job control은 Go mutex 하나로 직렬화하고 다른 Job control은 병렬로 처리한다.
