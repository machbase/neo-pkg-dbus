...
3. generic은 Job별 `_dbu_<jobName>` service를 유지한다. LS는 package 단위 Go daemon service `_dbu_collector` 하나를 사용하고, Job Start/Stop은 daemon의 논리 Job control로 처리한다.
...
8. 여러 관리자의 장기 화면 잠금은 제공하지 않는다. 대신 같은 Job의 POST create/start/stop, PUT update, DELETE와 package stop/uninstall을 하나의 mutation operation으로 직렬화한다. Backend는 상태 조회부터 Controller side effect와 설정 파일 변경 완료까지 Job별 `operation lock`을 유지하고, 다른 mutation이 lock을 보유하면 HTTP 409 `JOB_CONFLICT`를 반환한다. GET/list/status/last-run/validate와 DataViewer/Log 조회는 mutation lock을 잡지 않는다. Job GET 응답의 정수 `revision`은 호환을 위한 정보이며, Backend는 lock 안에서 다시 읽은 server revision을 기준으로 저장한다.
...
| CCR-060 | 모든 target에서 Job마다 JSH collector service를 실행하고, cycle 완료 뒤 interval을 더해 다음 작업을 예약했다. | LS는 linux/amd64 Go daemon `_dbu_collector` 하나만 service로 실행한다. Job마다 reader goroutine을 두고, PLC 로컬 Unix epoch의 `00:00:00` 기준 fixed-rate tick으로 실행한다. interval은 LS에서 1ms 이상 정수이며 겹친 tick과 queue-full batch는 항상 skip한다. reader들은 하나의 bounded queue에 넣고 native appender를 소유한 writer goroutine 하나만 저장한다. Generic은 이번 범위에서 기존 JSH service와 1000ms 최솟값을 유지한다. | Go PLC 실증에서 정상 Go read/parse/append 경로가 약 4ms였고 JSH보다 충분히 빨랐다. 동시에 `dbus.SystemBus()` shared connection을 cycle마다 닫으면 다중 Job socket이 끊기는 결함을 발견해 Job별 persistent dedicated connection으로 수정·재검증했다. | 승인됨 — 2026-08-31 사용자: LS 우선 Go reader/writer, single writer/shared queue, always skip, 00:00:00 기준, Generic은 LS 후 별도 branch |
| CCR-061 | 통합 build 정책은 Git에 generic 완성 산출물만 commit한다고 정했다. | 현재 LS 납품 branch/release는 GitHub clone 또는 ZIP으로 즉시 설치할 수 있도록 `npm run build -- --target=ls` 결과, provider asset과 `cgi-bin/bin/neo-dbus-collector`를 commit한다. Go의 `collector-go/dist/`는 build 중간 산출물로 제외한다. Generic은 LS merge 뒤 별도 branch에서 만들고 검증한다. | PLC에는 Go compiler를 전제할 수 없으며, 다운로드 후 즉시 설치하려면 LS binary가 repository delivery artifact에 있어야 한다. ZIP의 실행 bit 손실은 install/runtime의 `chmod 0755` 보정으로 막는다. | 승인됨 — 2026-09-01 사용자: “LS배포용으로 빌드 결과가 commit되도록… clone 받으면 바로 사용가능” |
| CCR-062 | package lifecycle이 generic의 Job별 service 방식에 맞춰 기존 실행 Job을 각각 start/stop했고, LS daemon 재시작 뒤 logical Job의 started/stopped 상태를 복구할 저장 경계가 없었다. | LS package `install`은 `_dbu_collector` 하나를 자동 등록한다. package `start`/`stop`/`uninstall`은 각각 daemon 하나의 start/stop/unregister만 수행한다. logical Job Start/Stop은 `go-collector-active-jobs.json`의 active set과 Unix socket control을 함께 갱신하고, daemon 시작은 이 set을 복구한다. Job 삭제는 active set에서만 제거하며 마지막 Job이어도 daemon service는 유지한다. | package script는 package lifecycle이고 CGI Job action은 logical Job lifecycle이다. daemon 단위 service와 Job 단위 실행 의도를 분리해야 package restart와 service crash 뒤 stopped Job을 되살리지 않는다. | 승인됨 — 2026-09-01 사용자: package script/service와 Job lifecycle 분리, daemon install 자동 등록, started/stopped Job 구분 요구 |
...
| CCR-072 | LS Go daemon은 busy reader/full queue skip의 누적 횟수와 마지막 시각을 runtime checkpoint와 로그에만 기록했고, `/job/last-run`과 Job 상세에는 제공하지 않았다. logical Job Stop/Start도 이전 runtime 값을 남길 수 있었다. | `GET /job/last-run`의 `lastRun`은 LS에서 `overrunCount`와 `lastOverrunAt`을 추가로 반환한다. Job 상세 `LATEST RUN`은 `SKIPPED CYCLES`, `LAST SKIPPED`로 두 값을 5초 polling에 맞춰 표시하고, count가 0보다 크면 두 metric을 warning 색과 1px warning border로 강조한다. logical Job Start와 daemon 재시작은 해당 runtime 값을 초기화한다. queue-full 진단인 `queueSkipped`은 공개하지 않는다. | busy reader로 정해진 tick을 실행하지 못한 횟수와 마지막 발생 시각을 운영자가 즉시 확인해야 한다. checkpoint에 이미 값이 있고 API projection만 누락되어 있었다. | 승인됨 — 2026-09-01 사용자: “skip건수 monitoring 화면에 표시”, “skip검수 표시할때 마지막 시간까지 표시해야 한다.” |
| CCR-073 | LS GetDeviceData가 DBus `uint16` DataCount를 지원해도 공통 generic validator의 Call 1,000 Tag, Job 10,000 Tag, 512 KiB Job JSON/CGI request body 제한이 먼저 적용되어 큰 연속 PLC 주소 읽기를 만들 수 없었다. | LS는 generic JSH collector의 저장 버퍼 제한을 상속하지 않는다. 한 Call의 `DataCount`와 한 cycle 전체 Tag 행은 DBus `uint16` 최대 65,535까지 허용하고, Tag 설정을 담는 LS Job JSON은 16 MiB, create/update/validate request body는 envelope 여유를 포함해 17 MiB까지 허용한다. generic의 설정 제한은 바꾸지 않는다. | LS Go data-plane은 shared bounded queue와 단일 native writer를 사용하며, `DataCount`의 실제 ABI 상한은 uint16이다. 10,001 Tag 검증 테스트로 기존 10,000 한계를 넘는 Job이 통과함을 확인한다. | 승인됨 — 2026-09-01 사용자: “읽는 최대 메모리 번지수 10,000개 해제”, “확인 필요” |
| CCR-074 | 실행 중 Job은 모든 수정이 금지되어 Logging Controls도 현재 설정 표시만 했다. | LS Job 상세의 `LOG LEVEL`은 select와 `Apply`를 제공한다. `PUT /job/log?name=`은 정확히 `{revision, level}`만 받고, 일반 Job PUT와 같은 Job/interface mutation lock을 거쳐 canonical 설정과 index를 저장한다. 실행 중 LS Job은 Go daemon의 `refresh-log` control로 해당 canonical Job의 log policy만 hot-apply한다. reader, DBus connection, writer, queue, fixed-rate scheduler, runtime checkpoint와 overrun 값은 재시작하거나 초기화하지 않는다. 정지 Job은 저장만 하고 다음 Start에서 읽는다. generic build는 이 endpoint를 `LOG_HOT_APPLY_NOT_AVAILABLE`로 거부하며 화면에도 편집 control을 제공하지 않는다. | 로그레벨 변경을 위해 collector를 `stop → start`하면 DBus 연결과 수집 주기가 끊기고 skip monitoring이 초기화된다. Go daemon은 log policy를 별도 mutex 보호 map에서 읽으므로 해당 map만 바꿀 수 있다. | 승인됨 — 2026-09-01 사용자: “DBUS는 monitoring에서 수정”, “재시작 때문에… DBUS에서는 가능?”; CCR-084에서 canonical direct-read로 갱신 |
| CCR-075 | PLC 수집기는 정상 cycle마다 INFO/DEBUG를 남기고 Job별 `maxFiles`에 의존해 로그 용량·보존을 정했다. 제한된 PLC 저장공간에서 반복 오류/skip도 과도한 로그를 만들 수 있었다. | `settings.json.logging`을 전역 정책으로 추가한다: 기본 `maxFileBytes:1048576`, `maxFiles:3`, `summaryIntervalMs:3600000`. active log 하나와 회전 `.log` 최대 3개만 보존하며 archive/gzip 파일은 만들지 않는다. 정상 INFO는 collector/service 시작·종료와 설정 적용만 기록한다. 첫 실패/skip은 즉시 ERROR/WARN, 같은 window의 반복 실패/skip과 성공·저장 행 수는 한 줄 summary로 기록한다. DEBUG는 summary만 기록한다. Job `log.maxFiles`는 기존 문서 호환 입력으로만 유지하고 실제 rotation은 전역 정책이 우선한다. Frontend는 Logging Controls에서 전역 rotation을 읽기 전용으로 표시한다. | 수집 주기와 무관하게 PLC 디스크 사용량을 예측 가능하게 하고, 원인 분석에 필요한 최초 사건과 누적 상태를 보존한다. “압축”은 archive가 아니라 로그 내용을 요약하는 뜻이다. | 승인됨 — 2026-09-01 사용자: “정상적인 경우 시작 종료 정도”, “매 시간이나 read 건수마다 summary”, “archive로 압축이 아니라 내용을 축약해서 summary로 기록” |
| CCR-076 | LS 값은 모두 unsigned로 저장하고 interval은 단순 1ms 이상 입력만 받았다. | LS Tag에 `signed:boolean`(기본 `false`)를 추가한다. `%MX` Bit은 signed 변환을 하지 않는다. `%MB/%MW/%MD/%ML`은 signed일 때 각각 8/16/32/64-bit two's-complement로 바꾼 뒤 transform을 적용한다. CSV는 기존 `name,bias,multiplier,order`를 계속 읽고 선택 `signed` 5열(`true/false`, `1/0`)도 읽는다. LS는 설정 내부값 `ls.interval.useTaskCycle`을 기본 활성화하며 `ls.plc.program.GetTaskCycleInfo(uint16:0)`의 성공 응답 `period-ms`를 cycle로 쓴다. 저장/생성 시 interval은 해당 cycle의 양의 배수이고, 실패·비활성화면 cycle 1ms를 쓴다. 새 Job의 기본 interval은 10ms 이상인 가장 가까운 배수다. 기존 Job은 읽기/실행을 막지 않는다. | PLC DBus는 unsigned로 반환하지만 주소 자료형은 부호 해석이 필요한 Tag가 있다. Main Task의 검증된 TaskNumber는 `0`, 주기는 `period-ms`다. 기존 Job을 새 validation으로 막으면 배포 후 운영 Job이 사라지는 문제가 있어 save 경계에서만 강제한다. | 승인됨 — 2026-09-02 사용자: signed 옵션·변환 순서, TaskNumber 0/`period-ms`, cycle 배수 정책 확정 |
| CCR-077 | LS writer queue의 크기·flush 경계가 명시되지 않았고, Database Server의 기본 mapping이 Job별 복사본과 달라질 수 있었다. | LS는 하나의 Database Server profile과 하나의 persistent native appender를 공유한다. reader는 DBus decode/transform 뒤 bounded queue에 batch를 넣고 즉시 다음 cycle을 준비하며, writer만 append/flush한다. 내부 `settings.json.ls.writer` 기본값은 `queueCapacity:64`, `flushMaxRows:1024`, `flushIntervalMs:1000`이고 FE에는 노출하지 않는다. queue가 가득 찬 cycle도 항상 skip이며 `overrunCount`에 포함하고 `queueSkipped` subset으로 API/monitoring에 함께 제공한다. 제어된 Job/package stop은 queue drain 뒤 final flush하며, 비정상 종료에서는 마지막 flush 뒤 buffered data를 잃을 수 있다. LS Job의 Database summary는 읽기 전용이고 Database profile 수정은 실행 중 Job reload 확인을 거친다. | 저사양 PLC에서 appender 동시 실행을 피하고 native append의 간헐적인 flush pause를 reader scheduler와 분리한다. 하나의 table/profile을 강제하면 writer와 schema ownership도 하나로 유지된다. | 승인됨 — 2026-09-03 사용자: shared queue/single writer, internal queue·flush policy, single Database profile |
| CCR-078 | table 이름만 가진 이전 Database profile은 기존 table을 쓰는 새 Job에서 빈 Value Column 오류를 냈다. | `defaultTable`이 있고 `valueColumn`이 비어 있으면 `VALUE`를 기본 mapping으로 해석하며, 새 profile 저장과 기본 localhost profile에는 이를 명시적으로 저장한다. `stringValueColumn` 기본은 계속 빈 문자열이고 basetime column 이름은 저장하지 않고 table metadata에서 찾는다. 기존 table의 설정 column이 없거나 자료형이 다르면 Job validation은 `problem`, table, column을 반환하고 FE는 안내 toast와 해당 Database Server 편집 모달을 연다. 자동 생성한 default table도 profile에 실제 `VALUE`와 선택 `STR_VALUE`를 저장한다. | 표준 자동 생성 schema와 profile 기본값을 일치시키고, 사용자 schema가 다를 때 잘못된 column으로 수집을 시작하지 않기 위함이다. | 승인됨 — 2026-09-10 사용자 Issue 1 및 “Time 컬럼명은 빼고 Value 컬럼명만 VALUE” 확정 |
| CCR-079 | LS Backend는 uint16 DataCount 65,535를 허용하지만 화면에서 PLC 주소 자료형별 실용적인 연속 읽기 상한이 없었다. | LS FE의 한 Call DataCount는 `%MX/%MB` 4,096, `%MW` 2,048, `%MD` 1,024, `%ML` 512로 제한한다. 시작 address를 포함한 개수이므로 `%MB1000`, 4,096개의 마지막 Tag는 `MB5095`다. DeviceString type을 바꿔 현재 값이 상한을 넘으면 즉시 새 상한으로 줄이고 Tag를 다시 계산한다. Backend와 Go의 uint16 65,535 계약은 유지하며 generic은 바꾸지 않는다. | 총 byte 규모를 4,096 정도로 맞추면서 화면 설정 실수를 방지하고, API 호환성과 향후 진단 입력 가능성은 유지하기 위함이다. | 승인됨 — 2026-09-10 사용자 Issue 2 및 “화면에서만 제한, backend 65535 유지” 확정 |
| CCR-080 | PLC 성능 조사에 DBus·parse·writer queue 지연의 장기 통계가 없었다. | 내부 `settings.json.ls.performance` 기본값은 `{enabled:true,jobSampleCount:1000,writerSummaryIntervalMs:30000}`이다. 너무 작은 표본 수와 writer 주기는 각각 500과 10,000ms로 적용하고 Job 시작 log에 configured→applied 보정을 남긴다. Job summary는 `job=<name>`과 DBus 합계 `min/p50/avg/p99/max`, parse 평균, interval 초과 수, 오류 수를 microsecond 단위 한 줄로 기록한다. 공유 writer summary에는 Job 이름을 붙이지 않고 `scope=shared`로 표시하며 `maxQueueDepth`, append+flush `busyRatio`, enqueue부터 성공 Flush까지 `queueToDurableP99`를 기록한다. 각 로그 줄의 공통 prefix가 기록 시각을 제공한다. durable 표본은 최근 4,096개로 제한하고 raw cycle log는 만들지 않는다. 설정은 FE/API에 노출하지 않는다. | 원본 값·매 cycle log 없이 간헐적 DBus 지연과 writer 병목을 구분하고 PLC CPU·디스크 부담을 제한하기 위함이다. | 승인됨 — 2026-09-10 사용자 Issue 3, 통계 항목·단위·기본 주기·표시 scope 확정 |
| CCR-081 | Job Save를 연속 제출하거나 Log Level 같은 별도 설정이 revision을 먼저 증가시키면, 단일 운영자가 유효한 수정을 하는 경우에도 stale revision `JOB_CONFLICT`가 발생할 수 있었다. 또한 PLC Docker `/work` mount의 JSH directory rename `ENOENT`로 완료된 mutation lock이 남아 후속 저장이 계속 충돌할 수 있었다. | Job form의 Save/Create는 평소에 활성화하고 요청을 시작한 즉시부터 완료될 때까지만 비활성화한다. 동기 `savingRef`로 렌더 전 중복 submit도 무시하고 실패하면 버튼을 다시 활성화한다. 외부 PUT의 revision은 정보성 호환 필드로만 받고 저장 충돌 조건으로 사용하지 않는다. Backend는 Job mutation lock 안에서 읽은 최신 server revision의 다음 번호를 발급하고, 동일 config 재요청은 쓰기와 revision 증가 없이 성공한다. Canonical operation lock은 `wx`로 생성하는 `0600` owner JSON 파일로 변경하고 token 확인 뒤 unlink한다. 짧은 acquire/reclaim guard directory는 생성 자체를 lock으로 보지 않고 고정 `owner.json`을 `wx`로 획득하며 file unlink와 best-effort rmdir만 사용한다. Directory rename은 사용하지 않는다. Repository CAS와 Job/package/interface lock의 실제 동시 mutation `JOB_CONFLICT`는 유지한다. | 단일 운영자 LS 패키지에서 사용자 수정을 손실시키지 않으면서, 중복 클릭과 CGI/lifecycle의 실제 경쟁은 각각 FE guard와 Backend lock으로 막고 Docker directory rename 의존성을 제거하기 위함이다. | 승인됨 — 2026-09-10 Issue 4 수정 후, 2026-09-11 revision을 내부 순서 관리로만 사용하고 Save를 요청 중에만 비활성화하는 정책으로 변경. PLC에서 `wx` 동시 생성을 5회 검증해 매회 한 프로세스만 성공함을 확인하고 file lock으로 전환. Neo ZIP installer의 Docker directory rename 수정은 완료됐으나 아직 정식 release 전 |
| CCR-082 | LS Call의 DataCount가 커지면 수천 개 Tag 이름·Signed·Transform 입력이 한꺼번에 DOM에 생성되어 화면이 지나치게 길어지고 브라우저 렌더링 부담이 커졌다. | 전체 `selection.tags` 배열과 저장 형식은 유지하되 선택 Call 화면은 실제 배열 길이를 기준으로 50개씩만 렌더링한다. First/Previous/직접 페이지 입력/Next/Last pager는 Tag 수와 무관하게 항상 표시한다. 페이지 직접 입력은 Enter 또는 blur에 적용하고 유효 범위로 clamp한다. 행 번호와 수정 대상은 전체 배열 인덱스이며 DataCount 감소 시 현재 페이지를 새 마지막 페이지로 보정하고 Call 변경 시 첫 페이지로 초기화한다. CSV Import, validation과 Save는 현재 페이지가 아니라 전체 배열에 적용한다. 페이지 상태는 payload에 저장하지 않으며 generic은 바꾸지 않는다. | 현재 LS 화면 상한뿐 아니라 향후 더 큰 배열에서도 DOM 행 수를 50개로 제한하면서, 페이지 이동 때문에 보이지 않는 Tag 설정이 누락되거나 저장 형식이 달라지지 않게 하기 위함이다. | 승인됨 — 2026-09-11 사용자: 전체 배열 유지·화면 부분 수정·전체 배열 저장, 50개 시작, pager 항상 표시, CSV 전체 배열 적용 확정 |
| CCR-083 | PLC에서 긴 start/stop CGI 요청이 취소되면 operation lock이 남을 수 있는데, Neo JSH 8.5.11의 `process.kill(pid, 0)`는 종료 PID에 Node식 `ESRCH` 예외 대신 code 없는 `GoError: ... os: process already finished` 값을 반환했다. Backend가 이를 살아 있는 PID로 해석해 30초 lease 뒤에도 stale lock을 영구 회수하지 못했고, Side switch의 재클릭은 진행 중 요청을 abort해 이 상태를 만들 수 있었다. | PID probe의 `ESRCH`뿐 아니라 JSH의 명시적 `no such process`·`process already finished` 결과를 종료 PID로 판정하되 그 밖의 code 없는 GoError와 EPERM은 fail-closed로 유지한다. timer 해제 오류가 나도 현재 token의 file lock 제거는 반드시 시도한다. Side의 start/stop은 한 요청이 끝날 때까지 모든 Job switch를 비활성화하고 동기 ref로 렌더 전 재클릭도 무시한다. 정상 작업의 lock은 즉시 해제하고, 취소·비정상 종료 lock은 기존 30초 lease 뒤 자동 회수한다. | 단일 운영 UI의 중복 클릭이 CGI를 중단시켜 영구 `JOB_CONFLICT`를 만들지 않으면서 실제 동시 mutation과 PID 판정 불확실성은 계속 차단하기 위함이다. | 승인됨 — 2026-09-11 PLC `job-1`에서 종료된 PID 4075/4203의 lock과 무기한 409를 재현하고 JSH 반환값을 실측함 |
| CCR-084 | 수천 Tag를 포함한 LS Job JSON을 list, monitoring polling, health, Start/Stop 때마다 JSH가 읽고 검증해 응답이 느리고 CGI lock 보유 시간이 길어졌다. Go daemon runtime checkpoint도 매 cycle 파일을 교체해 hot path에서 불필요한 I/O를 만들었다. | LS의 완전한 설정 authority는 계속 `conf.d/jobs/<name>.json` 하나다. JSH는 저장 마지막에 `conf.d/job-index/<name>.json`이라는 작은 관리 index를 쓰며, full JSON과 index가 모두 있을 때만 등록 완료 Job으로 취급한다. list/health/monitoring/Start/Stop/package lifecycle/Interface·Method 참조는 index를 사용한다. 새 `GET /job/status?name=`은 index의 요약과 daemon service/runtime/last-run을 한 요청으로 반환하고, 편집 `GET /job`과 Tag hierarchy만 full JSON을 읽는다. LS Start/Stop JSH는 Job 설정·DB metadata를 읽지 않고 이름만 Go Unix socket으로 전달하며, Go가 Start/Reload에서 full JSON과 index revision을 검증한다. active Job 파일은 Go만 갱신한다. runtime checkpoint는 memory update 뒤 전용 goroutine이 최대 250ms 단위로 합쳐 쓰고 Start/Stop/Clear에는 즉시 durable write한다. 같은 Job의 Go control은 mutex로 직렬화한다. | 큰 Tag 배열을 관리·polling hot path에서 제거하고, JSH는 관리와 신호 전달만 담당하며 Go reader/writer가 data-plane과 실행 검증을 소유하게 한다. | 승인됨 — 2026-09-11 사용자: Tag 분리 대신 full JSON+작은 index, JSH 관리/Go 실행, 경량 status, checkpoint writer, 하위호환 미지원 확정 |
| CCR-085 | index 도입 이전 Job 또는 저장 중단으로 full JSON만 남은 Job을 자동 해석하면 등록 완료 여부와 revision을 확정할 수 없고 Tag 설정 중복·유실 위험이 있다. | 자동 migration과 이전 형식 하위호환은 제공하지 않는다. full JSON만 있으면 list에서 숨기고 직접 조회는 `JOB_INVALID_CONFIG`와 `This Job configuration cannot be read. Recreate the Job before continuing.`을 표시한다. 같은 이름 Create는 lock 안에서 orphan full/index를 정리하고 새 Job으로 만든다. Create/Update는 full JSON을 원자 저장한 뒤 index를 마지막에 저장한다. Update는 먼저 기존 index를 제거하므로 중단 상태는 full-only가 된다. Delete는 full을 먼저 지우고 index를 지운다. index-only 항목은 canonical full이 없으면 list에서 무시한다. 테스트·개발 배포 전에는 기존 Job을 모두 지우고 다시 생성한다. | 불완전 저장을 정상 Job으로 오인하지 않고 복잡한 migration 코드 없이 명확한 commit marker를 유지하기 위함이다. | 승인됨 — 2026-09-11 사용자: 개발 단계이므로 기존 Job 삭제·재생성, orphan same-name Create는 이전 원본 삭제 확정 |
| CCR-086 | CCR-085의 Update는 새 index를 쓰기 전에 기존 index를 삭제했고, FE도 편집 경로가 바뀌면 진행 중 PUT을 abort했다. 저장 직후 다른 Job을 고르는 특정 타이밍에 CGI가 중단되면 canonical Job만 남아 이후 `Job was not found.` 또는 등록 누락이 발생할 수 있었다. | Job Save/Create가 시작되면 Side와 Main은 메모리 기반 15초 save lease를 공유한다. 저장 중 선택한 Job은 마지막 한 건을 보류하고 저장 종료 뒤 연다. lease가 만료되면 화면 잠금을 자동 해제하고 목록을 다시 읽으며, 저장 PUT 자체는 경로 전환이나 form unmount로 abort하지 않고 늦은 화면 반영만 무시한다. Backend Update와 Log Level Update는 기존 index를 지우지 않고 canonical full JSON을 원자 저장한 뒤 index를 원자 덮어쓴다. index 기록이 실패하면 이전 index가 남아 Job 등록은 유지되고, 같은 config 저장을 재시도하면 revision을 올리지 않고 stale/invalid index를 canonical document에서 복구한다. 이전부터 index가 전혀 없는 full-only Job은 계속 CCR-085의 미지원 형식으로 처리한다. | UI 잠금은 사용자 실수를 줄이는 보조 수단일 뿐 브라우저 종료·네트워크 단절을 막을 수 없다. 기존 index를 보존하면 CGI가 어느 지점에서 끝나도 Job 이름이 사라지지 않고, idempotent 재시도로 canonical/index를 수렴시킬 수 있다. 단위 테스트에서 index write 실패 뒤 canonical revision 2·기존 index revision 1·상세 조회 가능 상태와 동일 저장 재시도 후 index revision 2 복구를 확인했다. | 승인됨 — 2026-09-14 사용자: 저장 중 선택의 영구 잠금 위험을 지적한 뒤, 요청 계속 실행·보류 선택·시간 제한 잠금·Backend 안전장치 구조에 “이 구조가 맞을 것 같다. 이렇게 수정” 승인 |
| CCR-087 | LS writer는 첫 Batch를 받은 뒤에야 native Appender를 연결했다. `%MB0` 4,096 Byte, 12ms 단독 시험에서 시작 구간 queue가 64까지 차고 queue-full skip 38회가 발생했다. `Appender.Connect`와 첫 durable flush만 선행한 신규 TAG 재시험에서도 queue-full skip 11회가 남았지만 같은 TAG 재시작은 0회여서, 실제 원인은 첫 DATA append가 수행하는 대량 TAG 자동 등록임을 확인했다. | logical Job Start의 writer 준비 단계에서 `Connect`, column/batch policy 설정 후 Table METADATA의 기존 TAG 이름을 한 번 읽는다. Job의 unique TAG 중 누락된 이름만 prepared `INSERT INTO <table> METADATA (<primary>) VALUES (?)`로 명시 등록한다. 기존 이름은 건너뛰고 여러 Job의 등록은 단일 writer가 직렬화한다. 준비·조회·등록 실패 시 Job은 stopped 상태를 유지한다. 그 뒤 첫 epoch-aligned 실제 batch를 append하고 즉시 durable flush할 때까지 반복 schedule을 열지 않는다. dummy row와 고정 sleep은 사용하지 않는다. | 대량 신규 TAG의 one-time 비용을 bounded queue로 흡수하다 데이터를 버리지 않고, TAG 등록 완료를 reader 시작의 명시적 선행조건으로 만들기 위함이다. Appender 소유권은 계속 단일 writer에 있고 steady-state queue-full은 기존 통계로 탐지한다. | 승인됨 — 2026-09-15 사용자: 비교시험 결과 확인 후 “신규 Tag라면 Tag를 먼저 등록하고 데이터 입력” 방침 승인. PLC 8.5.11 신규 TAG 4,096개/12ms 112초 시험에서 6,998 cycle·28,663,808 rows, queue-full 0; 2 Job 동시 수집에서도 각 queue-full 0 확인 |
| CCR-088 | CCR-087의 동기 준비는 신규 TAG 4,096개에서 약 5초 동안 Start 요청을 붙잡고, TAG SQL 등록을 단일 writer 안에서 수행하여 이미 수집 중인 다른 Job의 append까지 정지시킬 수 있었다. 준비 중 Monitoring은 이전 `LAST RUN`만 보여 요청이 진행 중인지 알기 어려웠다. | LS Start는 설정 검증, desired-active 저장, runtime `starting` checkpoint까지만 기다린 뒤 성공을 반환한다. 백그라운드 goroutine이 Appender 준비와 누락 TAG 등록을 마치면 `running`으로 바꾸고 reader를 시작한다. TAG 등록은 별도 SQL connection에서 `(server, table)`별로 직렬화하여 단일 Appender writer를 막지 않는다. `starting` 중 Stop은 context를 취소하며, 실패는 desired-active를 제거하고 `failed/stateDetail`에 실제 사유를 남긴다. FE는 `STARTING — Preparing tags…`와 spinner를 표시하고 1초 polling하며 Start/Edit/Delete를 막되 Stop은 허용한다. `LAST RUN` 시각은 실제 read/store 전에는 갱신하지 않는다. 프로그램은 별도의 “로그 확인” 안내를 표시하지 않는다. | 긴 초기화를 HTTP/CGI 요청 수명과 분리하면서도 TAG 준비 전 read 금지 계약을 유지하고, 신규 Job 준비가 기존 Job의 writer와 queue에 영향을 주지 않도록 하기 위함이다. | 승인·PLC 검증 완료 — 2026-09-15 사용자 방침 확정. Neo 8.5.11 PLC에서 신규 TAG 4,096개 Start는 0.383초에 `STARTING`을 반환하고 약 1초 뒤 `RUNNING`이 됐다. 준비 중 기존 323 TAG Job의 `lastStoredAt`은 계속 증가했고 양쪽 `queueSkipped`는 0이었다. `STARTING` 직후 Stop은 0.355초에 성공하여 `STOPPED`, `lastRun=null`, active checkpoint 제외가 유지됐다. Neo 컨테이너 재시작 뒤 active Job은 단일 collector process에서 자동 복구되어 저장을 재개했다. |
...
- 수정 설계: 모든 process 접근은 `require('process')`로 통일한다. PID 확인 함수가 `false`를 반환하거나 반환·예외의 `code`가 `ESRCH`일 때만 종료로 보고 stale lock을 회수한다. `EPERM`과 알 수 없는 오류는 생존 여부를 확정할 수 없으므로 `JOB_CONFLICT`로 보호한다. 정상 Job lock은 canonical owner JSON 파일의 `heartbeatAt`으로 만료를 판단하며 heartbeat는 token 재확인 뒤 임시 파일을 canonical 파일로 원자 교체한다. crash로 owner JSON이 완성되지 않은 경우에만 JSH의 `stat.mtime.unixMilli()`를 fallback으로 쓴다. 따라서 `utimesSync`와 `mtimeMs` 의존성을 제거한다.
...
위 예시는 `conf.d/jobs/production-line.json`에 저장되는 document다. document top-level `name`은 파일명 `production-line`과 반드시 같아야 한다. `methodCalls`는 한 개 이상이다. 각 Call은 고정 ID, 표시 이름, `interfaceId`, `methodId`, raw inputs, `outputSelections`를 가진다. Tag는 `name`, `bias`, `multiplier`, `signed`를 가진다. `signed`가 없는 기존 Tag는 `false`다. LS에서 `%MX` Bit은 signed를 무시하고, `%MB/%MW/%MD/%ML`만 해당 bit width의 two's-complement 변환 뒤 transform을 적용한다. Tag 배열 순서가 값 연결 순서다. `sourceAddress`, `calcOrder`, `outputIndex`는 예전 Job을 읽을 때만 입력으로 허용하고 다음 저장에서 제거한다. Tag name은 TAG table의 `NAME VARCHAR(100)`에 맞춰 최대 100자다. `log.maxFiles`는 기존 Job 문서 호환 필드이며 실제 보존 개수는 `settings.logging.maxFiles`다.

`database.valueColumn`은 숫자 저장에 사용하는 column이고 `database.stringValueColumn`은 선택 문자열 column이다. 문자열 column을 쓰지 않을 때는 `""`로 저장한다. `database.table`은 항상 대문자 SQL 식별자로 저장한다. 기본 `localhost`는 실제 Table을 미리 만들지 않고 `defaultTable: "DEFAULT_DBUS"`, `valueColumn: "VALUE"`, 빈 `stringValueColumn`으로 시작한다. Database Server에 table 이름만 저장해도 Value Column은 `VALUE`로 보완하며 basetime 이름은 저장하지 않는다. Database Server 저장은 Default Table을 만들거나 metadata를 검증하지 않는다. Job POST 또는 정지된 Job PUT은 mutation lock 안에서 Table 존재 여부를 항상 확인한다. 기존 Table과 생성 경쟁 결과는 primary/basetime, 설정 Value/String Column 존재와 자료형을 검증하되 schema를 변경하지 않는다. 없는 Table은 최종 Output Mapping에 문자열·JSON·object·array 저장이 하나라도 있으면 `VALUE`와 `STR_VALUE`, 없으면 `VALUE`만 포함한 TAG Table로 만들며 config도 각각 `VALUE`/`STR_VALUE` 또는 `VALUE`/빈 문자열로 정규화한다. 생성 실패 시 Job config·service는 바꾸지 않으며 `/job/validate`와 runtime Start는 Table을 만들지 않는다. 자동 생성 VALUE는 DOUBLE이다. 기존 integer VALUE column은 collector가 metadata 자료형에 맞춰 exact integer로 append하며, 계산 결과가 fractional이거나 범위를 벗어나면 해당 cycle을 오류로 처리한다. `%ML`의 정확한 64-bit 저장에는 사용자가 알맞은 integer table을 만들어야 한다. 숫자 output은 Tag의 `transformOrder`를 왼쪽부터 적용해 `valueColumn`에 저장한다. 기본 `["bias", "multiplier"]`는 `(value + bias) * multiplier`이고, 반대 순서는 `(value * multiplier) + bias`다. 문자열·JSON·복합 output은 Transform 없이 String Value Column이 있을 때 저장하며 같은 행의 숫자 value에는 `0`을 저장한다. String Value Column이 없으면 해당 문자열 계열 행만 버리고 같은 Method·cycle의 숫자 행은 계속 저장한다. 이 생략은 `DB_APPEND_FAILED`가 아니다. DataViewer Chart는 숫자 column만 사용하고 Grid는 선택 문자열 column이 있을 때 문자열 값을 함께 보여 준다.
...
start는 외부에서 service가 지워진 Job이면 내부적으로 설치한 뒤 시작한다. `STARTING`과 `STOPPING`은 완료될 때까지 새 lifecycle 요청을 받지 않는다. 일반 update/delete가 실행 중이면 HTTP 409과 `JOB_RUNNING`을 반환한다. 단, LS의 Log Level 전용 `PUT /job/log`은 CCR-074의 범위에서 실행 중에도 허용한다. 사용자 DBus Interface를 Job이 하나라도 참조하면 이름 외 수정, Discover, Method 변경, 삭제를 `DBUS_INTERFACE_IN_USE`로 거부한다.
...
Package stop은 목록에서 상태를 아는 모든 configured Job을 `stopForPackage()`로 검사한다. 단, package start checkpoint에는 stop 직전 `RUNNING`, `STARTING`, `STOPPING`이었던 Job만 저장한다. Package lifecycle은 Job lock 일부만 잡다가 충돌해도 이미 잡은 lock과 fence를 모두 풀고 실패한다. 이 실패는 `PACKAGE_LIFECYCLE_FAILED`이며 원래 `JOB_CONFLICT`를 `details.errors[]`에 보존하므로 호출자가 안전하게 재시도할 수 있다. API mutation은 읽기 전용 availability probe로 fence 상태를 확인하고 Job lock을 잡은 뒤 같은 probe로 다시 확인한다. Probe는 canonical fence가 없거나 lease가 지났고 owner PID가 종료됐으면 available로 판단하며 파일이나 guard를 만들거나 회수하지 않는다. Fresh fence, 살아 있는 owner, PID 판단 오류는 `JOB_CONFLICT`다. 실제 stale 회수는 exclusive acquire만 수행한다. API가 Job lock을 잡은 뒤 replacement fence가 보이면 두 번째 probe가 실패하고, package가 먼저 fence를 잡고 API가 가진 Job lock에 도달하면 package 쪽이 실패하므로 둘이 함께 side effect를 실행할 수 없다. Package session 해제는 성공한 Job handle만 제거하고 실패한 handle과 fence는 보존한다. 모든 Job handle이 해제된 뒤에만 fence를 풀며, 실패한 같은 `session.release()`를 재시도할 수 있다.

canonical lock은 디렉터리가 아니라 `0600` JSON 파일이며 `owner token`, `pid`, `acquiredAt`, `heartbeatAt`을 가진다. `openSync(path, "wx")`의 `O_CREAT | O_EXCL`로 하나의 CGI만 원자 생성한다. Neo JSH 8.5.11은 exclusive collision을 `EEXIST`가 아니라 `ENOENT`로 포장하므로 실패 뒤 실제 path가 있으면 충돌로 판정한다. heartbeat는 임시 파일을 완성한 뒤 canonical **파일**을 원자 교체한다. owner는 CGI 요청이 끝날 때 heartbeat를 중지하고 canonical을 다시 읽어 자기 token일 때만 unlink한다. 이전 owner는 새 owner 파일을 갱신하거나 삭제할 수 없다.
...
owner schema는 비어 있지 않은 `token`, 0보다 큰 safe integer `pid`, 유한한 `acquiredAt`/`heartbeatAt`, `heartbeatAt >= acquiredAt`을 요구한다. schema가 잘못됐거나 JSON이 완성되지 않은 canonical 파일은 incomplete로 취급한다. fresh incomplete는 `JOB_CONFLICT`로 보호하고 파일 mtime 기준 lease가 지난 뒤에만 PID 검사 없이 orphan으로 회수한다. 유효 owner는 lease가 지나고 PID 종료가 `false` 또는 `ESRCH`로 확정된 경우에만 회수한다. `EPERM`, 검사 예외, 기타 불명 상태는 계속 보호한다.

acquire와 stale 회수 사이의 교체 경쟁은 짧게 존재하는 `<canonical>.reclaim` directory로 직렬화한다. Neo JSH의 `mkdirSync`는 기존 directory에도 성공할 수 있으므로 directory 생성 자체를 lock으로 사용하지 않는다. 모든 contender가 고정 `owner.json`을 `wx`로 생성하며 정확히 하나만 guard를 얻는다. stale takeover는 `owner.json` **파일**만 token별 claim 파일로 rename하여 다른 contender를 계속 차단하고, 해제는 자신이 소유한 파일을 unlink한 뒤 `rmdir`한다. directory rename은 사용하지 않는다. canonical 획득 뒤 guard 정리 실패는 획득한 canonical을 무효화하지 않고 best-effort로 끝낸다. 남은 guard는 다른 mutation을 잠시 막을 수 있지만 lease가 지나면 같은 회수 규칙으로 제거된다. fresh/live/ambiguous guard는 `JOB_CONFLICT`이고 stale dead 또는 stale incomplete guard만 같은 token/entry를 재확인해 정리한다. guard는 정상적으로는 canonical 생성 또는 stale 전환 직후 제거되며 실제 Controller/config mutation 동안 유지되지 않는다.

이전 배포판의 directory canonical lock도 호환해서 읽는다. fresh/live directory owner는 그대로 보호하고, lease 만료와 dead PID가 확인되면 guard 안에서 기존 entry를 정리하고 같은 경로에 file lock을 생성한다. 정상 file lock 획득·release와 legacy 회수에는 directory rename이 없다. PLC Docker `/work` 실측에서는 두 JSH 프로세스의 `wx` 동시 생성을 5회 반복해 매회 정확히 하나만 성공하고 패자의 쓰기가 기존 내용을 바꾸지 않음을 확인했다.
...
| PUT | `/job/log?name=` | `{revision, level}`; LS만 | 수정된 Job |
...
- PUT body는 name 없는 부분 config patch이며 호환을 위해 `revision`을 포함할 수 있다. client revision은 생략하거나 현재보다 낮아도 저장을 막지 않는다. Job lock 안에서 읽은 server revision을 기준으로 실제 변경에만 1 증가하고, 동일 config 재요청은 증가 없이 성공한다. 병합 순서는 `defaults → existing config → request patch`다. 객체는 깊이 병합하고 배열은 request 배열 전체로 교체한다.
...
7. `DESIGN.md`의 디자인 토큰과 접근성 규칙을 지키며, 동작 상태를 설명하는 문서화된 motion 예외만 사용한다.
