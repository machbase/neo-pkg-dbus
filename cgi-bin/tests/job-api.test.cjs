...
function run() {
  ...
  try {
    ...
    fs.writeFileSync(path.join(cgiRoot, 'conf.d', 'interfaces', 'device-status.json'), JSON.stringify({
      schemaVersion: 1, id: 'device-status', builtIn: false,
      busType: 'system', destination: 'ls.plc', objectPath: '/ls/plc/device', interface: 'ls.plc.device',
      methods: [{ id: 'get-device-data', source: 'manual', member: 'GetDeviceData',
    ...
    assert.equal(response.payload.data.configState, 'installed');
    assert.equal(response.payload.data.executionState, 'stopped');
    ...
    assert.equal(descriptor.enable, false);
    ...
    const removedServiceState = JSON.parse(fs.readFileSync(stateFile, 'utf8'));
    delete removedServiceState._dbu_alpha;
    fs.writeFileSync(stateFile, JSON.stringify(removedServiceState));
    response = runCgi(path.join(cgiRoot, 'api', 'job', 'start.js'), moduleRoot, stateFile, {
      method: 'POST', query: 'name=alpha',
    });
    assertEnvelope(response, 200, true);
