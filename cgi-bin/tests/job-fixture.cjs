...
function setupRoot(prefix) {
  ...
  writeJson(path.join(root, 'conf.d', 'interfaces', 'device-status.json'), {
    ...
    methods: [{ id: 'get-device-data', source: 'manual', member: 'GetDeviceData',
      inputs: [{ name: 'dataCount', type: 'uint16', validation: { minimum: 1, maximum: 65535 } }, { name: 'memoryAddress', type: 'string' }], outputs: [{ name: 'result', type: 'string' }] }],
  });
  return root;
}
