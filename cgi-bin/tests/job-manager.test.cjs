...
async function testHappyLifecycleAndProjection() {
  ...
  try {
    ...
    assert.deepEqual({
      ...
      configState: 'config-only', executionState: 'stopped', controllerState: 'NOT_INSTALLED',
      statusKnown: true, installed: false, running: false,
    });
    assert.equal(service.calls[0][0], 'status', 'create는 쓰기 전에 orphan service를 확인해야 합니다.');
    assert.equal(service.calls.some(([method]) => method === 'install'), false);
    assert.equal(database.calls.length, 1);

    const installed = await call(manager, 'install', 'line_a-01');
    assert.equal(installed.controllerState, 'STOPPED');
    ...
    service.details.set('_dbu_line_a-01', {
      lastRun: {
        ...
        profileVersion: 1,
        methodCalls: [{
          id: 'read-plc-data-1', name: 'Read PLC', requestedAt: 'a', completedAt: 'b',
    ...
    assert.deepEqual(lastRun, {
      lastRun: {
        ...
        status: 'success', profileId: 'ls-electric-plc', profileVersion: 1,
        methodCalls: [{
          id: 'read-plc-data-1', name: 'Read PLC', requestedAt: 'a', completedAt: 'b',
          status: 'success', storedCount: 2, error: null,
        }],
    ...
    assert.deepEqual({
      ...
      name: 'line_a-01', profileId: 'ls-electric-plc', destination: 'ls.plc',
...
async function testUpdateSerializesStartAcrossManagers() {
  ...
  try {
    ...
    await call(managerA, 'install', 'alpha');
...
async function testDatabaseGuardsAndWarnings() {
  ...
  try {
    ...
    database.recover();
    const created = await call(manager, 'create', { name: 'alpha', config: jobConfig() });
    await call(manager, 'install', 'alpha');
    database.fail(dbFailure);
...
async function testInvalidIdentityAndRollback() {
  ...
  try {
    ...
    service.setInstallState({});
    await rejectsCode(call(manager, 'install', 'rollback'), 'CONTROLLER_UNKNOWN');
    assert.equal(service.states.has('_dbu_rollback'), false, '실패한 install은 service를 보상 정리해야 합니다.');
    assert.equal(fs.existsSync(path.join(root, 'conf.d', 'jobs', 'rollback.json')), true);

    service.setInstallState({ status: 'STOPPED' });
    await call(manager, 'install', 'rollback');
    ...
    for (const action of ['install', 'start', 'stop', 'update', 'delete']) {
...
async function testPackageStopHoldsEarlierJobLockUntilWholeLifecycleCompletes() {
  ...
  try {
    ...
    for (const name of ['alpha', 'beta']) {
      ...
      await call(packageManager, 'install', name);
...
async function testPackageUninstallFencesDeletedAndNewJobNamesUntilCompletion() {
  ...
  try {
    ...
    await call(packageManager, 'install', 'beta');
