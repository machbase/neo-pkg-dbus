...
function fixture(options) {
  ...
  const connection = {
    query(sql, ...values) {
      ...
      if (sql.includes('_TAG_META')) return [{ _ID: 1, [primaryColumn]: '%MB3' }, { _ID: 2, [primaryColumn]: '%MB4' }];
...
async function testJobScopedDataViewer() {
  ...
  assert.equal(numericPage.rows[0].grid.value, '12.5');
...
async function run() {
  ...
  assert.deepEqual(await call(created.viewer, 'createTable', { server: 'local-db', table: 'TAG_NEW' }), {
    server: 'local-db', table: 'TAG_NEW', created: true,
  });
  const ddl = created.calls.find((entry) => entry.type === 'exec');
  assert.match(ddl.sql, /^CREATE TAG TABLE TAG_NEW /);
  assert.match(ddl.sql, /STR_VALUE VARCHAR\(1024\)/);
  await rejectsCode(call(created.viewer, 'createTable', { server: 'local-db', table: 'TAG;DROP' }), 'DB_REQUEST_INVALID');
  ...
  assert.deepEqual(await call(tags.viewer, 'tags', { server: 'local-db', table: 'TAG', limit: '1' }), {
    server: 'local-db', table: 'TAG', tags: [{ name: '%MB3' }], limited: true, limit: 1,
  });
  ...
  assert.deepEqual(await call(alternateRoles.viewer, 'tags', { server: 'local-db', table: 'TAG' }), {
    server: 'local-db', table: 'TAG', tags: [{ name: '%MB3' }, { name: '%MB4' }], limited: false, limit: 200,
  });
  const alternateTagQuery = alternateRoles.calls.find((entry) => entry.type === 'query' && entry.sql.includes('_TAG_META'));
  assert.match(alternateTagQuery.sql, /SELECT _ID, TAG_ID FROM _TAG_META ORDER BY TAG_ID LIMIT \?/);
  ...
  assert.match(alternateDataQuery.sql, /SELECT _RID, TAG_ID, TS, VALUE, STR_VALUE FROM TAG WHERE TAG_ID = \?/);
  assert.match(alternateDataQuery.sql, /ORDER BY TS DESC, _RID DESC LIMIT \?, \?/);
  ...
  assert.deepEqual(alternateChart.series, [{ name: '%MB3', points: [{ time: '2026-08-03T00:00:00.000Z', value: 12.5 }] }]);
  const alternateChartQuery = alternateRoles.calls.filter((entry) => entry.type === 'query' && entry.sql.includes('FROM TAG')).at(-1);
  assert.match(alternateChartQuery.sql, /SELECT TS, TAG_ID, VALUE FROM TAG WHERE TAG_ID IN \(\?\) ORDER BY TS ASC, TAG_ID ASC LIMIT \?/);
  ...
  assert.deepEqual(page.rows.find((row) => row.name === '%MB3'), {
    ...
    grid: { name: '%MB3', time: '2026-08-03T00:00:00.000Z', value: '12.5' },
  });
  ...
  const offsetRows = {
    '%MB3': [5, 4, 3, 2, 1].map((rid) => ({
      _RID: rid, NAME: '%MB3', TIME: new Date('2026-08-03T00:00:05.000Z'), VALUE: rid, STR_VALUE: null,
    })),
    '%MB4': [3, 2, 1].map((rid) => ({
      _RID: rid, NAME: '%MB4', TIME: new Date(`2026-08-03T00:00:0${rid}.000Z`), VALUE: rid * 10, STR_VALUE: null,
    })),
  };
  const offsetFixture = fixture({
    dataRows({ tag, sql, values }) {
      assert.match(sql, /ORDER BY TIME (ASC|DESC), _RID \1 LIMIT \?, \?/);
      const ascending = /ORDER BY TIME ASC, _RID ASC/.test(sql);
      const ordered = offsetRows[tag].slice().sort((left, right) => {
        const timeOrder = left.TIME.getTime() - right.TIME.getTime();
        const ridOrder = left._RID - right._RID;
        return ascending ? (timeOrder || ridOrder) : -(timeOrder || ridOrder);
      });
      return ordered.slice(values.at(-2), values.at(-2) + values.at(-1));
    },
  });
  ...
  const chart = fixture();
  const series = await call(chart.viewer, 'chart', {
  ...
  assert.equal(series.job, 'line-a');
  assert.deepEqual(series.columns, { primary: 'NAME', time: 'TIME', numericValue: 'VALUE' });
  assert.deepEqual(series.series, [{ name: '%MB3', points: [{ time: '2026-08-03T00:00:00.000Z', value: 12.5 }] }]);
  ...
  const specialSeries = await call(specialChart.viewer, 'chart', {
    job: 'line-a', names: specialNames,
  });
  assert.deepEqual(specialSeries.series, [
    { name: '__proto__', points: [{ time: '2026-08-03T00:00:00.000Z', value: 1 }] },
    { name: 'constructor', points: [{ time: '2026-08-03T00:00:01.000Z', value: 2 }] },
  ]);
