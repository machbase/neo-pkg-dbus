...
async function run() {
  ...
  try {
    ...
    assert.deepEqual(await call(reader, 'columns', await call(store, 'get', 'local-db'), 'TAG'), {
      ...
      tableType: 'TAG',
      columns: [
        { name: 'NAME', type: 'varchar(80)', primaryKey: true, basetime: false, numeric: false, string: true },
        { name: 'TIME', type: 'datetime', primaryKey: false, basetime: true, numeric: false, string: false },
        { name: 'VALUE', type: 'double', primaryKey: false, basetime: false, numeric: true, string: false },
        { name: 'STR_VALUE', type: 'varchar(256)', primaryKey: false, basetime: false, numeric: false, string: true },
      ],
    });
