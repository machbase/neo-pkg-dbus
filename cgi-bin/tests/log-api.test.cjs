...
async function run() {
  ...
  try {
    ...
    assert.deepEqual(Object.keys(firstPage).sort(), ['file', 'lines', 'linesPerPage', 'name', 'nextPage', 'page', 'previousPage', 'totalLines']);
    ...
    assert.deepEqual(Object.keys(entire).sort(), ['content', 'file', 'name', 'size']);
    assert.equal(entire.content.includes('secret'), false);
    assert.equal(entire.content.includes('raw-payload'), false);
    const tail = await call(reader, 'tail', { name: 'line-a', file: 'line-a.log', lines: '2' });
    assert.deepEqual(Object.keys(tail).sort(), ['file', 'lines', 'name', 'totalLines']);
