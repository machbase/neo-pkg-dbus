...
async function run() {
  ...
  try {
    ...
    const settings = new SettingsManager({ cgiRoot: root, profileStore: manager.store });
    ...
    const customDefaultSettings = await call(settings, 'update', {
      ...updatedSettings, defaultProfileId: 'custom-plc',
    });
    await assert.rejects(call(manager, 'deleteProfile', 'custom-plc'), (failure) => {
      assert.equal(failure.code, 'PROFILE_DEFAULT');
      assert.deepEqual(failure.details, { id: 'custom-plc', defaultProfileId: 'custom-plc' });
      return true;
    });
    await call(settings, 'update', { ...customDefaultSettings, defaultProfileId: 'ls-electric-plc' });
