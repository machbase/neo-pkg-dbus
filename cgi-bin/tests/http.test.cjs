...
assert.equal(http.statusForError({ code: 'JOB_DATA_SOURCE_MISMATCH' }), 400);
assert.equal(http.statusForError({ code: 'TABLE_INVALID' }), 400);
assert.equal(http.statusForError({ code: 'TABLE_ALREADY_EXISTS' }), 409);
...
assert.equal(http.statusForError({ code: 'DBUS_ARGUMENT_UNSUPPORTED' }), 409);
