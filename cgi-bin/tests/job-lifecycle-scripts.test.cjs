...
async function testStopAndUninstallPrintNormalFailureEnvelope(root) {
  for (const verb of ['stop', 'uninstall']) {
    ...
    manager.acquirePackageLifecycle = () => {
      throw Object.assign(new Error('alpha mutation is pending'), {
        code: 'JOB_CONFLICT', details: { name: 'alpha' },
      });
    };
    const lifecycle = createLifecycle(manager, path.join(root, 'data', `${verb}-stdout.json`), {
    ...
    assert.deepEqual(JSON.parse(printed[0]), {
      ...
      reason: '1개 Job의 package lifecycle 작업이 실패했습니다.',
      details: { errors: [{
        name: 'alpha', code: 'JOB_CONFLICT', reason: 'alpha mutation is pending', details: { name: 'alpha' },
      }] },
    });
