...
test('JSH stat에서 mtimeMs 접근 자체가 없어도 incomplete lock을 stale 회수한다', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'neo-job-operation-lock-no-mtimems-'));
  const timers = makeTimers();
  const lockDirectory = path.join(directory, 'alpha.lock');
  let clock = Date.now();
  try {
    ...
    const jshFs = {
      ...fs,
      statSync(target) {
        assert.equal(target, lockDirectory);
        return new Proxy({ mtime: { unixMilli: () => clock - 30001 } }, {
          get(value, key) {
            if (key === 'mtimeMs') throw new Error('JSH에는 stat.mtimeMs가 없습니다.');
            return value[key];
          },
        });
      },
    ...
    const handle = locks.acquire('alpha');
    assert.equal(handle.token, 'owner-b');
    handle.release();
  } finally {
    fs.rmSync(directory, { recursive: true, force: true });
  }
