...
function testMachbaseAppenderWithoutStringColumn() {
  ...
  database.append([{ name: 'NUM', requestTime, value: 1.5, stringValue: null }]);
  assert.deepEqual(appended, [['NUM', requestTime, 1.5, null]]);
  assert.throws(() => database.append([
    { name: 'TEXT', requestTime, value: 'READY', stringValue: 'READY' },
  ]), (failure) => failure.code === 'DB_APPEND_FAILED' && /stringValueColumn/.test(failure.message));
  database.close();
}
...
function testEntryStartupAndShutdown() {
  ...
  const runtime = startCollector({ name: 'alpha', ...jobConfig() }, {
    cgiRoot: '/tmp/cgi-bin',
    jobName: 'alpha',
    profileStore: { find(id) { assert.equal(id, 'ls-electric-plc'); return profile; }, isCompatible() { return true; } },
...
function testStartupFailuresStayAliveAndBackoff() {
  ...
  const dbus = {
    call() {
      ...
      return { body: ['{"rtn":1,"data-count":2,"data":[1,2]}'] };
    },
    close() {},
  };
  let runtime;
  assert.doesNotThrow(() => {
    runtime = startCollector({ name: 'alpha', ...jobConfig() }, {
      cgiRoot: '/tmp/cgi-bin', jobName: 'alpha', database, dbus,
      profileStore: { find() { return profile; }, isCompatible() { return true; } },
      settings: { limits: { maxGeneratedTagsPerCall: 1000, maxBufferedRowsPerCycle: 10000 } },
      validateConfig(config) { return config; },
      details: { setLastRun() {} },
...
function testCollectorLoggerAndLogApiIntegration() {
  ...
  try {
    ...
    assert.doesNotThrow(() => {
      const failingRuntime = startCollector({ name: 'alpha', ...jobConfig() }, {
        cgiRoot: path.join(root, 'cgi-bin'), jobName: 'alpha',
        profileStore: { find() { return profile; }, isCompatible() { return true; } },
        settings: { limits: { maxGeneratedTagsPerCall: 1000, maxBufferedRowsPerCycle: 10000 } },
        validateConfig(config) { return config; },
        database: { open() {}, append() {}, close() {} },
        dbus: { call() { return { body: ['{"rtn":1,"data-count":2,"data":[1,2]}'] }; }, close() {} },
