...
test("buildRawResultColumns hides the unconfigured optional string value column", () => {
    const columns = buildRawResultColumns([
        { time: "2026-06-01", name: "sensor.a", value: 12.5, stringValue: null },
    ], { hiddenKeys: ["stringValue"] });

    assert.deepEqual(columns.map((column) => column.key), ["time", "name", "value"]);
});
...
test("buildNeoWebTagAnalyzerMessage builds chart-group scoped Tag Analyzer payload", () => {
    ...
    assert.equal(built.message.appName, "neo-pkg-dbus");
